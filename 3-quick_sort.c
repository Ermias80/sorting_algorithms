...
/**
 * quicksort_lo - Recursive function for Quick sort with Lomuto
 * partition scheme
 ...
 * Return: Nothing
 */
void quicksort_lom(int *array, int left, int right, size_t size)
{
	int pivot_index;
	...
	if (left < right)
	{
		...
		quicksort_lom(array, left, pivot_index - 1, size);
		quicksort_lom(array, pivot_index + 1, right, size);
	}
}
/**
...
void quick_sort(int *array, size_t size)
{
	if (array == NULL || size < 2)
		return;
	quicksort_lom(array, 0, size - 1, size);
}
